...
proc sort data = actg out = actg;
 by tx;
run;
/*Fit an exponential distribution for the primary outcome \time to AIDS de_ning event or death" in each of the treatment groups (tx = 0; 1)*/
...
/*Assess whether the exponential distribution in part (1a) is reasonable*/
proc lifetest data=actg plots=logsurv notable;
strata tx;
time time*censor(0);
run;

/*Plot the Kaplan-Meier estimators of the survival functions for each of the treatment groups (tx = 0; 1) and use the log rank test to determine whether the survival experience for the two treatment
groups differs*/
...
/*Fit a Cox proportional hazards regression model with tx as the sole predictor and assuming one unspecified baseline hazard function*/
...
proc freq data=actg;
tables censor*tx/nopercent nocol norow;
run;
